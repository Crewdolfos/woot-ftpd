...
{
  if(state->logged_in){
      char err_msg[] = "633 Error: Insufficient permissions to execute command ";
      switch(lookup_arg(cmd->arg)){
        case CHMOD:                                                     
          if(strcmp(cmd->arg1, "")){
            char buf[128];
            sprintf(buf, err_msg);
            sprintf(buf + strlen(buf), "CHMOD\n");
            state->message = buf;
          }else{
            state->message = "214 CHMOD\tchange mode of a file. E.g. SITE CHMOD 755 filename\n";
          }
          memset(cmd->arg1,0,strlen(cmd->arg1));
          break;                                                       
        case GROUP:                                                    
          if(strcmp(cmd->arg1, "")){
            char buf[128];
            sprintf(buf, err_msg);
            sprintf(buf + strlen(buf), "GROUP\n");
            state->message = buf;
          }else{
            state->message = "214 GROUP\trequest special group access. E.g. SITE GROUP foo\n";
          }
          memset(cmd->arg1,0,strlen(cmd->arg1));
          break;
        case EXEC:
          if(strcmp(cmd->arg1, "")){
            char buf[1024];
            sprintf(buf, err_msg);
            sprintf(buf + strlen(buf), cmd->arg1);
            sprintf(buf + strlen(buf), "\n");
            state->message = buf;
          }else{
            state->message = "214 EXEC\texecute a program.  E.g. SITE EXEC program params\n";
          }
          memset(cmd->arg1,0,strlen(cmd->arg1));
          break;
        case GROUPS:                                                    
          if(strcmp(cmd->arg1, "")){
            char buf[128];
            sprintf(buf, err_msg);
            sprintf(buf + strlen(buf), "GROUPS\n");
            sprintf(buf, cmd->arg1);
            state->message = buf;
          }else{
            state->message = "214 GROUPS\tShows available groups.\n";
          }
          memset(cmd->arg1,0,strlen(cmd->arg1));
          break;
        case IDLE:                                                    
          if(strcmp(cmd->arg1, "")){
            ...
            state->message = "214 IDLE\tset idle-timer. E.g. SITE IDLE 60\n";
          }
          memset(cmd->arg1,0,strlen(cmd->arg1));
          break;
        case GPASS:                                                    
          if(strcmp(cmd->arg1, "")){
            char buf[128];
            sprintf(buf, err_msg);
            sprintf(buf + strlen(buf), "GPASS\n");
            sprintf(buf, cmd->arg1);
            state->message = buf;
          }else{
            state->message = "214 GPASS\tgive special group access password. E.g. SITE GPASS bar\n";
          }
          memset(cmd->arg1,0,strlen(cmd->arg1));
          break;
        case ALIAS:                                                    
          if(strcmp(cmd->arg1, "")){
            ...
            state->message = "214 ALIAS\tshow settings or list all defined ALIAS if no alias name was specified.\n";
          }
          memset(cmd->arg1,0,strlen(cmd->arg1));
          break;
        case CHECKSUM:                                                    
          if(strcmp(cmd->arg1, "")){
            char buf[128];
            sprintf(buf, err_msg);
            sprintf(buf + strlen(buf), "CHECKSUM\n");
            state->message = buf;
          }else{
            state->message = "214 CHECKSUM\tcalculates and displays checksum values, as does HASH.\n";
          }
          memset(cmd->arg1,0,strlen(cmd->arg1));
          break;
        default:                                                    
          state->message = "214-The following SITE commands are recognized (* =>'s unimplemented).\n"
                "CHMOD\tGROUP\tEXEC\tGROUPS\n"
                "IDLE*\tGPASS*\tALIAS*\tCHECKSUM\n"
                "214 Direct comments to support@Crewdolfos.pwn\n";
      }
  }else{
    state->message = "530 Please login with USER and PASS.\n";
  }
...
void ftp_help(Command *cmd, State *state) 
{
  ...
  state->message = "214-The following commands are recognized.\n"
    "USER\tPASS\tPASV\tLIST\tCWD\tPWD\tMKD\tRMD\n"
    "RETR\tSTOR\tDELE\tSITE\tSIZE\tABOR\tQUIT\tTYPE\n"
    "CDUP\tHELP\tNLST\tRNFR\tRNTO\tAPPE\tNOOP\n"
    "214 Direct comments to support@Crewdolfos.pwn\n";
  write_state(state);
}
