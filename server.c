  ...
  while(1){
    ...
    if(pid==0){
      ...
      if(strlen(welcome_message)<BSIZE-4){
        ...
        strcat(welcome, "(Version wu-2.5.0(1) Tue Sep 21 16:48:12 EDT 1999)");
      }
...
int lookup_arg(char *arg){
  const int arglist_count = sizeof(arglist_str)/sizeof(char *);
  return lookup(arg, arglist_str, arglist_count);
}
